	...
	def send(self) -> str:
		...
		message = self.model.invoke(
			[
				...
				HumanMessage(
					content='\n'.join(
						[str(m.content) for m in self.messages_history] + [self.prefix]
					)
				),  # put in whole message history for the agents to debate
			]
		)
		# self.messages records the messages from this dialogagent. self.messages_history records the whole message history.
		self.messages_history = operator.add(
			self.messages_history, [HumanMessage(content=f'{self.prefix}: {message}')]
		)
		self.messages.append('\n' + str(message.content))
		return str(message.content)

	def receive(
		self, name: str, message: Union[str, BaseMessage, Sequence[BaseMessage]]
	) -> None:
		"""
		Concatenates {message} spoken by {name} into message history
		"""
		self.messages_history = operator.add(
			self.messages_history, [HumanMessage(content=f'{name}: {message}')]
		)
		...
		message = AIMessage(
			content=agent_chain.run(
				input='\n'.join(
					...
					+ [str(m.content) for m in self.messages_history]
					+ [self.prefix]
				)
			)
		)
