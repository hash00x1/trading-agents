...
def calculate_trend_signals(prices_df):
	...
	ema_5 = calculate_ema(prices_df, 30).tail(
		84
	)  # 6 * 5 = 30, 5 days as EMA fast line, quick trend or bounce
	ema_14 = calculate_ema(prices_df, 84).tail(
		84
	)  # 6 * 14 = 84, 14 days as Core trend-following period
	ema_28 = calculate_ema(prices_df, 128).tail(
		84
	)  # 6 * 28 = 168, 28 days as EMA slow line, used to confirm trend bias
...
def calculate_mean_reversion_signals(prices_df):
	...
	price_vs_bb = (prices_df['close'].tail(period).iloc[-1] - bb_lower.iloc[-1]) / (
...
def calculate_volatility_signals(prices_df):
	...
	vol_z_score = (hist_vol - vol_ma) / hist_vol.rolling(90).std()
...
def calculate_rsi(
	prices_df: pd.DataFrame, period: int = 14, period_days: int = 84
) -> pd.Series:
...
def calculate_bollinger_bands(
	prices_df: pd.DataFrame, window: int = 20, period: int = 84
