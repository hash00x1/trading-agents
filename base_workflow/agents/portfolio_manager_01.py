...
# from base_workflow.utils.llm import call_llm


class PortfolioDecision(BaseModel):
    action: Literal["buy", "sell", "hold"]
    quantity: int = Field(description="Amount of cryptocurrency to trade (supports fractional quantities)")
    confidence: float = Field(description="Confidence in the decision, between 0.0 and 100.0")
    reasoning: str = Field(description="Reasoning for the decision")
    # token_pair: str = Field(description="The trading pair, e.g., 'BTC/USDT'")


class PortfolioManagerOutput(BaseModel):
    decisions: dict[str, PortfolioDecision] = Field(description="Dictionary mapping trading pair (e.g., 'BTC/USDT') to trading decisions")
...
def portfolio_manager(state: AgentState):
    ...
    progress.update_status("portfolio_manager", None, "Analyzing signals")
